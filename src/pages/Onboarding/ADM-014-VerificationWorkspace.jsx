...
import {
  ...
  selectManufacturerVerification,
...
export default function VerificationWorkspace() {
  ...
  const {
    ...
    decisionStatus,
    decisionError,
  } = useSelector(selectManufacturerVerification);
  ...
  useEffect(() => {
    dispatch(fetchManufacturerApplication(manufacturerId));
    return () => dispatch(clearManufacturerWorkspace());
  ...
  const handleSubmit = async () => {
    const result = await dispatch(
      decideManufacturerApplication({ id: manufacturerId, decision, reason: note }),
    );
    if (!result.error) {
      setDecision('');
      setNote('');
  ...
  if (viewState === 'error') {
    return (
      <ErrorState
        ...
        onRetry={() => dispatch(fetchManufacturerApplication(manufacturerId))}
      />
    );
  }
  if (viewState === 'empty' || !application) {
    return (
      <EmptyState
        ...
        actionLabel={t('onboarding.backToManufacturers')}
        onAction={() => navigate('/onboarding/applications')}
      />
    );
  }
    ...
    <SplitReviewLayout
      header={
        <PageHeader
          ...
          subtitle={`${application.legalName} · ${application.city} · ${formatRelativeTime(application.submittedAt)}`}
          meta={
      ...
      decision={
        <div className="flex flex-col gap-field">
          ...
          <dl className="flex flex-col gap-2 rounded border border-lightGray-dark bg-lightGray p-3 text-sm">
            ...
            <MetaRow
              label={t('onboarding.fieldBisLicence')}
              value={application.bisLicence ?? t('common.none')}
              mono
            />
            ...
            <MetaRow
              label={t('onboarding.fieldCategories')}
              value={application.categories.join(', ')}
            />
            <MetaRow
      ...
      footer={
        <div className="flex justify-end gap-2">
          <Button variant="secondary" onClick={() => navigate('/onboarding/applications')}>
            {t('common.cancel')}
          </Button>
          <Button
            disabled={!canSubmit}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="flex flex-wrap items-center gap-2 text-sm font-medium text-charcoal">
                  ...
                  {/* A failed blocking check is the reason approve is off, so
                      it says so rather than leaving the reviewer to guess. */}
